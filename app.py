"""
Stock Price API + User Auth - Market Brain v7
FastAPI backend with persistent login (1-year sliding token TTL)
"""
...
CACHE_TTL    = 5
TOKEN_TTL    = 60 * 60 * 24 * 365   # 1 year — sliding expiry keeps users logged in
REQUEST_TIMEOUT = 8
...
async def load_token(token: str) -> Optional[str]:
    """Load token and refresh its TTL (sliding expiry - active users stay logged in permanently)."""
    r = await get_redis()
    if r:
        try:
            val = await r.get(f"token:{token}")
            if val:
                # Refresh TTL on every use — users stay logged in as long as they're active
                await r.setex(f"token:{token}", TOKEN_TTL, val)
                return val
        except:
            pass
...
# ── App ───────────────────────────────────────────────────────
app = FastAPI(title="Market Brain API", version="7.0.0", lifespan=lifespan)
